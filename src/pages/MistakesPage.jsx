          ...
          {filtered.map(m => (
            <div key={m.id} className={`mj-card${m.reviewed ? " reviewed" : ""}`}>
              <div className="mj-card-head">
                <span className="mj-card-source">{SOURCE_LABELS[m.source] || m.source}</span>
                <span className="mj-card-date">{new Date(m.date).toLocaleDateString()}</span>
              </div>
              <div className="mj-card-prompt">{m.prompt}</div>
              <div className="mj-card-answers">
                ...
                <div className="mj-card-correct">
                  ...
                  <button className="conv-speak" onClick={() => speakThai(m.correctAnswer)} title="Listen">🔊</button>
                </div>
              </div>
